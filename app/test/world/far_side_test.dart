...
const _assets = 'assets/images/workers/7a19';

void main() {
  TestWidgetsFlutterBinding.ensureInitialized();

  test('far leg is a plain, UN-FLIPPED duplicate (byte-identical to the near leg)', () {
    for (final leg in ['leg-upper', 'leg-lower']) {
      final near = File('$_assets/$leg.png').readAsBytesSync();
      final far = File('$_assets/$leg-far.png').readAsBytesSync();
      expect(far, orderedEquals(near),
          reason: '$leg-far must be a plain copy — no flip, no baked tint (task 19k)');
    }
  });
  ...
  test('far arm stays behind the torso horizontally across the bob cycle', () async {
    await (TestWidgetsFlutterBinding.instance).runAsync(() async {
      Flame.images.clearCache();
      // A dummy sprite for every part — geometry comes from the manifest regions,
      // not the sprite pixels, so a 8×8 stand-in is fine.
      ...
      for (var i = 0; i < 40; i++) {
        ...
        for (final arm in ['arm-upper-far', 'arm-lower-far']) {
          ...
          expect(r.left, greaterThanOrEqualTo(torso.left - tol), reason: '$arm left pokes out at tick $i');
          expect(r.right, lessThanOrEqualTo(torso.right + tol), reason: '$arm right pokes out at tick $i');
        }
      }
    });
  });
