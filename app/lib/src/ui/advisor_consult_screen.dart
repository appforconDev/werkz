...
class AdvisorConsultScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AdvisorConsultScreen> createState() => _AdvisorConsultScreenState();
}
...
class _AdvisorConsultScreenState extends ConsumerState<AdvisorConsultScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) => ref.read(consultationProvider.notifier).ensureStarted());
  }
            ...
            if (plan != null && !s.waiting)
              Padding(
                ...
                child: FilledButton.icon(
                  style: FilledButton.styleFrom(backgroundColor: Werkz.approvalGreen, padding: const EdgeInsets.symmetric(vertical: 12)),
                  onPressed: () => showWorkOrderSheet(context, initialDirective: plan),
                  icon: const Icon(Icons.assignment_turned_in_outlined, size: 18),
                  label: const Text('DISPATCH THIS PLAN (FORM 17-B)',
                      style: TextStyle(fontFamily: Werkz.mono, fontWeight: FontWeight.w900, letterSpacing: 1)),
                ),
              ),
            _Composer(controller: _ctrl, enabled: !s.waiting, onSend: _send),
            const Padding(
              padding: EdgeInsets.only(bottom: 8, left: 14, right: 14),
              child: Text('Consulting the Advisor runs on your own Claude — no new cost, no new key.',
                  textAlign: TextAlign.center,
                  style: TextStyle(fontFamily: Werkz.mono, fontSize: 9, color: Werkz.steel)),
            ),
          ],
        ),
      ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: const [
            ...
            SizedBox(height: 12),
            Text('FILE A MEMO TO THE ADVISOR',
                style: TextStyle(fontFamily: Werkz.mono, fontWeight: FontWeight.w900, letterSpacing: 1, fontSize: 13, color: Werkz.machine)),
            SizedBox(height: 8),
            Text('Describe the job you have in mind. The Advisor reads the project and '
...
class _Correspondence extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Align(
      ...
      child: Container(
        margin: EdgeInsets.only(top: 8, left: isOp ? 40 : 0, right: isOp ? 0 : 40),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              child: Text(isOp ? 'MEMO · OPERATOR' : 'ADVISORY · RESPONSE',
                  style: TextStyle(fontFamily: Werkz.mono, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 1.5,
  ...
  Widget build(BuildContext context) {
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(color: Werkz.carbon, border: Border.all(color: Werkz.gunmetal, width: 1.5)),
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: const [
          ...
          SizedBox(width: 8),
          Flexible(
            child: Text('The Advisor is reading the file and drafting a response…',
                style: TextStyle(fontFamily: Werkz.mono, fontSize: 11, fontStyle: FontStyle.italic, color: Werkz.gunmetal)),
          ),
        ]),
      ),
    );
...
class _ErrorSlip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
      decoration: BoxDecoration(border: Border.all(color: Werkz.stampRed, width: 1.5), color: Werkz.cream),
      child: Row(children: [
        const Icon(Icons.error_outline, size: 16, color: Werkz.stampRed),
        const SizedBox(width: 8),
        Expanded(child: Text(error, style: const TextStyle(fontFamily: Werkz.mono, fontSize: 11, color: Werkz.stampRed))),
      ]),
    );
  }
}
...
class _Composer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(left: 12, right: 12, top: 4, bottom: MediaQuery.of(context).viewInsets.bottom + 4),
      child: Row(
        ...
        children: [
          ...
          FilledButton(
            ...
            child: const Text('FILE', style: TextStyle(fontFamily: Werkz.mono, fontWeight: FontWeight.w900, letterSpacing: 1)),
          ),
        ],
      ),
    );
