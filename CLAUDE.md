...
- **assets-pipeline/** — scripts calling gpt-image-2 via Fal. `FAL_KEY` from `.env` only. **Never commit keys; `.env` is gitignored, `.env.example` documents required vars.** **PIXEL WORK IS RICKARD'S (STANDING RULE, task 36).** Image cutout / freeing / simple image editing is done by RICKARD by hand, NOT via a pipeline auto-isolate. CC builds the code + pipeline; pixel work goes to him. Hand-cleaned files are PROTECTED auto-pickup inputs (convention: `*_clean.png` / `master-clean.png`) — if present they are used VERBATIM, always, ahead of any generated output; generators/isolators never overwrite them and skip when they exist; a hand-clean whose canvas mismatches the expected size (or lacks alpha) FAILS LOUDLY — never silently scaled.
